...
<?php
    ...
    if(isset($result)){
        if(($result['value'] == $_POST['code']) && ($result['date'] >= $_GET['date'])){
            ...
            redirectFailure('codes_verification.php', 'Le code ne correspond pas, merci de réessayer&token=' . htmlspecialchars($_GET['token']) .'&id=' . htmlspecialchars($_GET['id']) . '&check=0');
        }
